...
function addPlace(item,open_informer,center){
	try{
		...
		myPlacemark = new ymaps.Placemark([array[1],array[2]], {
			content: item['name'],
			balloonContent: "<b><span style='from-size:14px'>" +item['name']+ "</span></b> <a href='" +item['redir']+ "'>подробнее</a><br/><div style=\"float:left;\">" +item['addres']+ "</div><div style='float:right;padding-left: 10px;'><img width=\"50\" src='" +item['image']+ "' /></div>"
		});
		
		if(center) myMap.setCenter([array[1],array[2]],15);
