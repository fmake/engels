...
$globalTemplateParam->set('ignor_delete_security', $ignor_delete_security);
	...
	$news_categories = $absitem->getChilds($parent);
...
switch($request->action)
{
	...
	case 'new': // Далее форма
        ...
        if($news_categories)foreach($news_categories as $category){
            if($category['id'] != $items['id']) $_select->AddOption(new selectOption($category['id'], $category['title'], (($category['id'] == $items['parent'])? true : false )));
        }
