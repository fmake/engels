...
switch ($request->action) {
    ...
    default:
		...
		if($filters){
			//echo 'qq';
			$items = $absitem->getByPageAdminFilter($filters,$id_page_modul, $limit, $page,"a.`file` = 'item_expert'");
			$count = $absitem->getByPageCountAdminFilter($filters,$id_page_modul,$id_page_modul,"a.`file` = 'item_expert'");
		}else{
			$items = $absitem->getByPageAdmin($id_page_modul, $limit, $page,"a.`file` = 'item_expert'");
			$count = $absitem->getByPageCountAdmin($id_page_modul,$id_page_modul,"a.`file` = 'item_expert'");
		}
		//printAr($items);
    ...
    case 'new': // Далее форма
		...
		/**/
		$fmakeSiteUser = new fmakeSiteUser($items_dop['id_user']);
		$user_info = $fmakeSiteUser->getInfo();
		if ($user_info['post_create']) {
			if ($user_info['name']) {
				$name_user_expert = $user_info['name'];
			} else {
				$name_user_expert = $user_info['login'];
			}
		} else {
			if ($user_info['name']) {
				$name_user_expert = $user_info['name'];
			} else {
				$name_user_expert = $user_info['name_social'];
			}
		}
		/**/
		...
		$form->addHtml('Эксперт',"<td>Эксперт : </td><td><a href='/admin/?modul=adm_users_site&id={$items_dop['id_user']}&action=edit'>{$name_user_expert}</a></td>");
		// /admin/?modul=adm_users_site&id=137&action=edit
