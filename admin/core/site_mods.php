...
/*function setGeneralParent($from,$to){
	...
	$fmakeSiteModulRelation->setGeneralPageRelation($from,$to);	
...
function setParent($child,$parent){
	...
	$fmakeSiteModulRelation->setPageRelation($parent,$child);
...
if($admin->role == 3){
	echo $admin->role;
	$no_content = true;
	$globalTemplateParam->set('no_content', $no_content);
}
...
switch($request->action)
{
	...
	case 'new': // Далее форма
		...
		foreach ($filds as $key=>$fild)
		{
			...
			else $form->addHtml($fild,"<tr><td><em>{$fild}</em></td><td>".$items[$key]."</td></tr>");
		}
