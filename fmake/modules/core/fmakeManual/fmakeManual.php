...
class fmakeManual extends fmakeSiteModule {
	...
	function getChilds($id = null, $active = false, $inmenu = false,$where = false,$count_advert = false) {
		...
		if($count_advert){
			$join_table = " LEFT JOIN (SELECT `parent`,COUNT('*') as count FROM `site_modul` WHERE `active`='1' AND file='item_manual' GROUP BY parent) as b ON a.id = b.parent ";
			$fild = ",b.count";
			//echo 'qq';
		}	
		...
		if ($level != $level_vlojennost || !$level_vlojennost) {
			...
			if ($items) {
				foreach ($items as $item) {
					if ($item['delete_security'] || $item['file']=='item_manual')
						continue;
					$item['level'] = $level;
					$this->tree[] = $item;
	...
	function getConversionCountMultiParent ($items) 
	{
		$fmakeSiteModuleMultiple = new fmakeSiteModule_multiple();
		if ($items) foreach ($items as $key=>$item) {
			$items[$key]['count'] += sizeof($fmakeSiteModuleMultiple->getPagesParentId($item['id']));
		}
		return $items;
	}
	
	function getCatForMenu($parent = 0, $active = false, $inmenu = false,$count_advert = false) {
		$items = $this->getChilds($parent, $active, $inmenu, "`file` != 'item_manual'",$count_advert);
		//printAr($items);
		$items = $this->getConversionCountMultiParent($items);
	...
	function export_csv($filename,$afields,$table = false)
	{
		$dir = "/upload-files/";
		...
		$fmakeManual->table = $table;
		$fmakeManual->order = "id";
		$items = $fmakeManual->getAll();
		
		//echo sizeof($items)."<br/>";
		
		//$FileHandler = fopen($_SERVER['DOCUMENT_ROOT'].$dir.$filename, 'w+');
		if ($items) {
			foreach ($afields as $fild) {
				//$item[$fild] = mysql_real_escape_string($item[$fild]);
				//$item[$fild] = iconv("UTF-8", "Windows-1251", $item[$fild]);
				$str .= "{$fild}{$seperator}";
			}
			$str .= "\r\n";
			foreach ($items as $key=>$item) {
