<?php
	...
	class fmakeSiteModule_multiple extends fmakeCore
	{
		...
		function getPagesParentId($id_parent)
		{
			$select = $this->dataBase->SelectFromDB(__LINE__);
			
			$fmakeSiteModule = new fmakeSiteModule();
			
			$join_table = " LEFT JOIN ".$fmakeSiteModule->table." as b ON a.id_site_modul = b.".$fmakeSiteModule->idField." ";
			$fild = ",b.active";
			
			return $select->addFild("a.*".$fild)->addFrom($this->table." as a".$join_table)->addWhere("a.`id_parent`='" . $id_parent . "'")->addWhere("b.`active`='1'")->queryDB();
		}
	}
?>