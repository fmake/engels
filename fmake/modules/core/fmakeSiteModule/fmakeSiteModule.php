...
class fmakeSiteModule extends fmakeCore {
	...
	function getAllForMenuSite($parent = 0, $active = false, &$q, &$flag, $inmenu, $acces = false, $level = 0, $level_vlojennost = false, $type = false) {
		if ($level != $level_vlojennost || !$level_vlojennost) {
			...
			foreach ($items as $key => $item) {
				...
				if ($flag)
					$items[$key]['status'] = &$q;
				$items[$key]['child'] = $this->getAllForMenuSite($item['id'], $active, $q, $flag, $inmenu, $acces, $level++, $level_vlojennost, $type);
				if ($flag)
					unset($items[$key]['status']);
	...
	function getByPageAdmin($parent, $limit, $page, $where = "a.`file` = 'item_news'", $active = false) {
		...
		return $select->addFild("a.*,b.*,c.caption name_category")->addFrom($this->table . $table_join)->addWhere("b.`id`!=0")->queryDB();
		//return $select->addFild("a.parent,a.caption,a.title,a.text,a.redir,a.full_url,a.file,a.picture,a.date_create,a.position,a.active,b.*,c.caption name_category")->addFrom($this->table . $table_join)->addWhere("b.`id`!=0")->queryDB();
	}
	...
	function getByPageSearch($q, $limit, $page, $where = false, $active = false) {
		$select = $this->dataBase->SelectFromDB(__LINE__);
		if ($active)
			$select->addWhere("`active`='1'");

		if($q)
			$select->addWhere("`caption` LIKE '%{$q}%' OR `text` LIKE '%{$q}%'");
		if($where)
			$select->addWhere($where);
		if($this->order) 
			$select->addOrder($this->order, $this->order_as);
	...
	function getByPageCountSearch($q, $type = false, $where = false, $active = false) {
		$select = $this->dataBase->SelectFromDB(__LINE__);
		if ($active)
			$select->addWhere("`active`='1'");
...
function getBreadCrumbs($id)
	...
	function addFile($tempName, $name, $iswantermark = true) {
		...
		if ($iswantermark) {
			...
			$images->resize(406, false, true, $dirname, '406__', false);
		}
		...
		$images->resize(140, 100, true, $dirname, '140_100_', false);
