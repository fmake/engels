<?php
	...
	class fmakeSiteModule_tags extends fmakeCore{
		...
		*/
		...
		function getByPage($tag, $limit, $page, $where = false, $active = false) {
			$select = $this->dataBase->SelectFromDB(__LINE__);
			if ($active)
			...
			$table_join = " as a LEFT JOIN {$fmakeSiteModule->table} as b ON b.id = a.id_site_modul";

			if($where)
				$select->addWhere($where);
			if($this->order) 
					$select->addOrder($this->order, DESC);	
			return $select->addFild("a.*,b.*")->addFrom($this->table_notice_tags . $table_join)->addWhere("`{$this->idField}` = '{$tag}'")->addLimit((($page - 1) * $limit), $limit)->queryDB();
		}

		function getByPageCount($tag, $where = false, $active = false) {
			$select = $this->dataBase->SelectFromDB(__LINE__);
			if ($active)
				$select->addWhere("`active`='1'");
			...
			if($where)
				$select->addWhere($where);
			$result = $select->addFild("COUNT(*)")->addFrom($this->table_notice_tags . $table_join)->addWhere("`{$this->idField}` = '{$tag}'")->queryDB();
			return $result[0]["COUNT(*)"];
		}
