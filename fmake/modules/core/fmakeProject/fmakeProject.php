...
class fmakeProject extends fmakeSiteModule {
	...
	function getChilds($id = null, $active = false, $inmenu = false,$where = false,$count_advert = false) {
		...
		if($count_advert){
			$join_table = " LEFT JOIN (SELECT `parent`,COUNT('*') as count FROM `site_modul` WHERE `active`='1' AND file='item_project' GROUP BY parent) as b ON a.id = b.parent ";
			$fild = ",b.count";
			//echo 'qq';
		}	
		...
		if ($level != $level_vlojennost || !$level_vlojennost) {
			...
			if ($items) {
				foreach ($items as $item) {
					if ($item['delete_security'] || $item['file']=='item_project')
						continue;
					$item['level'] = $level;
					$this->tree[] = $item;
	...
	function getCatForMenu($parent = 0, $active = false, $inmenu = false,$count_advert = false) {
		$items = $this->getChilds($parent, $active, $inmenu, "`file` != 'item_project'",$count_advert);
		//printAr($items);
		if (!$items)
			return;
