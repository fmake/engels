...
class fmakePlace extends fmakeSiteModule {
	...
	function setSearch($search_string,$category,$parent, $limit, $page, $where = "a.`file` = 'item_place'", $active = true) {
	...
	function setSearchCount($search_string,$category,$parent, $where = "a.`file` = 'item_place'", $active = true) {
		$select = $this->dataBase->SelectFromDB(__LINE__);
		if ($active)
			$select->addWhere("a.active='1'");
	...
	public function getScriptCat($id_cat){
		...
		if($select)foreach($select as $key=>$item){
			...
			$absitem_dop->setId($item['id']);
			$item_dop = $absitem_dop->getInfo();
			$item['caption'] = addslashes($item['caption']);
