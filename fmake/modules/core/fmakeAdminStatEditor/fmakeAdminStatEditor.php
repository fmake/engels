...
class fmakeAdminStatEditor extends fmakeSiteModule {
	...
	function getStats($filters = false) {
		...
		if($all_users)foreach($all_users as $key=>$item){
			...
			$result[$key]['kol_vo_news'] = $fmakeNews->countNewsEditor($item['id'],$filters);
			$result[$key]['sr_kol_vo_simvolov_news'] = $fmakeNews->averageNewsSymvolEditor($item['id'],$filters);
			$result[$key]['sr_kol_vo_uniq_prosm'] = $fmakeNews->countNewsViewEditor($item['id'],$filters);
