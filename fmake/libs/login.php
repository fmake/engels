...
if($request->code){
	...
	$redir_url = "http://engels.bz{$item['full_url']}";
	
	$vk = new VKapi();
	$vk->login($request->code);
	$fb = new FBapi();
	$fb->login($request->code,$redir_url);
	
	
	header("HTTP/1.1 301 Moved Permanently");
	if ($item['full_url']) header('Location: '.$redir_url); 
...
switch ($request->action){
	...
	case 'logout':
		...
		$item = $fmakeSiteModule->getInfo();
		header("HTTP/1.1 301 Moved Permanently");
		if ($item['full_url']) header('Location: http://engels.bz'.$item['full_url']); 
		else header('Location: /');
