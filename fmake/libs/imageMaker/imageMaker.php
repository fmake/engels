...
class ImageMaker {
	...
	function wantermark_img($images,$url_wantermark) {
		require_once 'api.watermark.php';
		$watermark = new watermark();
		$main_img_obj = imagecreatefromjpeg($images);
		$watermark_img_obj	= imagecreatefrompng($url_wantermark);
		$return_img_obj			= $watermark->create_watermark( $main_img_obj, $watermark_img_obj, 100 );
		imagejpeg( $return_img_obj,$images, 50 );
	} 
	//$w = ширина фотки
	//$h = высота фотки
	...
	//$wantermark = wmi|images/watermark.png|BL
	function resize($w=false,$h=false,$obrezanie_foto=false,$folder=false,$prefix='',$format=false,$wantermark = false){
		...
		if($wantermark)$trumb->fltr['wmi'] = $wantermark;
	...
	//fltr[]=wmi|images/watermark.png|BL
